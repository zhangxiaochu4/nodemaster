...
namespace QtNodes
{
    ...
    NodeGraphicsObject::NodeGraphicsObject(BasicGraphicsScene &scene, NodeId nodeId) : _nodeId(nodeId), _graphModel(scene.graphModel()), _nodeState(*this), _proxyWidget(nullptr)
    {
        scene.addItem(this);
    ...
    void NodeGraphicsObject::mousePressEvent(QGraphicsSceneMouseEvent *event)
    {
        ...
        for (PortType portToCheck : {PortType::In, PortType::Out})
        {
            ...
            if (!connected.empty() && portToCheck == PortType::In)
            {
                ...
                NodeConnectionInteraction interaction(*this, *nodeScene()->connectionGraphicsObject(cnId), *nodeScene());
            ...
            else // initialize new Connection
            {
                ...
                {
                    auto const outPolicy = _graphModel.portData(_nodeId, portToCheck, portIndex, PortRole::ConnectionPolicyRole).value<ConnectionPolicy>();
                ...
                ConnectionId const incompleteConnectionId = makeIncompleteConnectionId(_nodeId, portToCheck, portIndex);
    ...
    void NodeGraphicsObject::hoverMoveEvent(QGraphicsSceneHoverEvent *event)
    {
        ...
        if ((_graphModel.nodeFlags(_nodeId) | NodeFlag::Resizable) && geometry.resizeHandleRect(_nodeId).contains(QPoint(pos.x(), pos.y())))
        {
