...
namespace QtNodes
{
    ...
    static QPointF computeAverageNodePosition(QJsonObject const &sceneJson)
    {
        ...
        for (QJsonValueRef node : nodesJsonArray)
        {
            ...
            averagePos += QPointF(nodeJson["position"].toObject()["x"].toDouble(), nodeJson["position"].toObject()["y"].toDouble());
        }
    ...
    CreateCommand::CreateCommand(BasicGraphicsScene *scene, QString const name, QPointF const &mouseScenePos) : _scene(scene), _sceneJson(QJsonObject())
    {
    ...
    DeleteCommand::DeleteCommand(BasicGraphicsScene *scene) : _scene(scene)
    {
        ...
        QJsonArray connJsonArray;
        //首先删除所选的连接，确保在删除所选节点时不会自动删除它们(删除节点也会删除一些连接)。
        ...
        QJsonArray nodesJsonArray;
        //删除节点;这将删除许多连接。在此循环之前，选中的连接已经被删除，
        for (QGraphicsItem *item : _scene->selectedItems())
        {
            if (auto n = qgraphicsitem_cast<NodeGraphicsObject *>(item))
            {
    ...
    void offsetNodeGroup(QJsonObject &sceneJson, QPointF const &diff)
    {
        ...
        for (QJsonValueRef node : nodesJsonArray)
        {
            ...
            QPointF oldPos(obj["position"].toObject()["x"].toDouble(), obj["position"].toObject()["y"].toDouble());
    ...
    PasteCommand::PasteCommand(BasicGraphicsScene *scene, QPointF const &mouseScenePos) : _scene(scene), _mouseScenePos(mouseScenePos)
    {
    ...
    QJsonObject PasteCommand::makeNewNodeIdsInScene(QJsonObject const &sceneJson)
    {
        ...
        for (QJsonValueRef connection : connectionJsonArray)
        {
            ...
            ConnectionId newConnId{mapNodeIds[connId.outNodeId], connId.outPortIndex, mapNodeIds[connId.inNodeId], connId.inPortIndex};
    ...
    DisconnectCommand::DisconnectCommand(BasicGraphicsScene *scene, ConnectionId const connId) : _scene(scene), _connId(connId)
    {
        //
    }
    ...
    ConnectCommand::ConnectCommand(BasicGraphicsScene *scene, ConnectionId const connId) : _scene(scene), _connId(connId)
    {
        //
    }
    ...
    MoveNodeCommand::MoveNodeCommand(BasicGraphicsScene *scene, QPointF const &diff) : _scene(scene), _diff(diff)
    {
