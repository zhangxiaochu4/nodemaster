...
namespace QtNodes
{
    ...
    ConnectionGraphicsObject::ConnectionGraphicsObject(BasicGraphicsScene &scene, ConnectionId const connectionId)
        : _connectionId(connectionId), _graphModel(scene.graphModel()), _connectionState(*this), _out{0, 0}, _in{0, 0}
    {
        scene.addItem(this);
    ...
    {
        // 这个函数只在新创建ConnectionGraphicsObjectis时被调用。此时两端坐标为(0,0)，与G.O.坐标相连接。
        // 整个连接g.o.在场景坐标系中的位置也是(0,0)。通过移动整个对象到节点端口位置我们正确定位两个连接端。
        ...
        if (_connectionState.requiredPort() != PortType::None)
        {
            ...
            if (ngo)
            {
                ...
                QPointF pos = geometry.portScenePosition(nodeId, attachedPort, portIndex, nodeSceneTransform);
    ...
    void ConnectionGraphicsObject::move()
    {
        auto moveEnd = [this](ConnectionId cId, PortType portType)
        {
            ...
            if (ngo)
            {
                ...
                QPointF scenePos = geometry.portScenePosition(nodeId, portType, getPortIndex(portType, cId), ngo->sceneTransform());
