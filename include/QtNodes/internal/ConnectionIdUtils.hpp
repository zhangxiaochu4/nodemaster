...
namespace QtNodes
{
    //返回connectionid对应的porttype的nodeid
     ...
     */
    inline ConnectionId makeIncompleteConnectionId(NodeId const connectedNodeId, PortType const connectedPort, PortIndex const connectedPortIndex)
    {
        return (connectedPort == PortType::In) ? ConnectionId{InvalidNodeId, InvalidPortIndex, connectedNodeId, connectedPortIndex} :
     ...
     */
    inline ConnectionId makeIncompleteConnectionId(ConnectionId connectionId, PortType const portToDisconnect)
    {
    ...
    {
        ostr << "(" << connectionId.outNodeId << ", " << (isPortIndexValid(connectionId.outPortIndex) ? std::to_string(connectionId.outPortIndex) : "INVALID") << ", "
             << connectionId.inNodeId << ", " << (isPortIndexValid(connectionId.inPortIndex) ? std::to_string(connectionId.inPortIndex) : "INVALID") << ")" << std::endl;
    ...
    {
        ConnectionId connId{static_cast<NodeId>(connJson["outNodeId"].toInt(InvalidNodeId)), static_cast<PortIndex>(connJson["outPortIndex"].toInt(InvalidPortIndex)),
                            static_cast<NodeId>(connJson["intNodeId"].toInt(InvalidNodeId)), static_cast<PortIndex>(connJson["inPortIndex"].toInt(InvalidPortIndex))};
